...
#ifdef DEBUGG_RUN
std::cout << "EntityManager :: COMPONENT_REGISTERED :: " << m_cFactory.size() << std::endl;
#endif
...
EntityManager::~EntityManager()
{
    ...
    #ifdef DEBUGG_RUN
        std::cout << "EntityManager :: END :: "<< m_entities.size() << std::endl;
    #endif
}
...
int EntityManager::AddEntity(const Bitmask& l_mask)
{
 ...
 //       std::cout << "ENTITY :: " << entity << std::endl;
    ...
    if( m_entities.find( entity ) == m_entities.end() ){
        m_entities[entity] = EntityData(0,ComponentContainer());
...
int EntityManager::AddEntity(const std::string& l_entityFile)
{
    ...
    std::ifstream file;
    file.open(Utils::GetWorkingDirectory() + "res/ents/" + l_entityFile);
    if( !file.is_open())
    {
        #ifdef DEBUGG_RUN
            std::cout << "EntityManager :: Failed to load entity: " << l_entityFile << std::endl;
        #endif
    ...
    while(std::getline(file,line))
    {
        ...
        if(type == "Name")
        {
            #ifdef DEBUGG_RUN
                keystream >> type;
                std::cout << "EntityManager :: ADD_ENTITY :: " << type <<std::endl;  
